...
mbd_loglik <- function(
  ...
  if (any(is.infinite(pars))) {
    return(-Inf) #stop("'pars' cannot contain Infs")
  }
  if (lambda < 0.0) {
    return(-Inf) #stop("'lambda' must be positive")
  }
  if (mu < 0.0) {
    return(-Inf) #stop("'mu' must be positive")
  }
  if (nu < 0.0) {
    return(-Inf) #stop("'nu' must be positive")
  }
  if (q < 0.0) {
    return(-Inf) #stop("'q' must be positive")
  }
  if (q > 1.0) {
    return(-Inf) #stop("'q' must be less or equal to one")
  }
  if (minimum_multiple_births < 0) {
    return(-Inf) #stop("'minimum_multiple_births' must be positive")
  }
  if (!mbd:::check_brts_consistency(brts = brts, soc = soc)) {
    return(-Inf) #stop("these branching times cannot be generated by a mbd process")
  }
  if (q >= 0 - safety_threshold) {
    return(-Inf)
  }
  if (q <= 1.0 + safety_threshold) {
    return(-Inf)
  }
