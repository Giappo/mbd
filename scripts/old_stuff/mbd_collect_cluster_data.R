...
get_all_data_from_cluster = function(sim_pars, max_sims = 1000, account_name = "p274829"){
  ...
  bad_results <- tidy_results[
    rowSums(tidy_results[,1:(Npars + 1)] == rep(-1,(Npars + 1))) == (Npars + 1),
  ]
  results <- tidy_results[
    rowSums(tidy_results[,1:(Npars + 1)] == rep(-1,(Npars + 1))) != (Npars + 1),
  ]
  print(paste("There are ", dim(bad_results)[1]," bad results.", sep = ''))
  print(paste("There are ", dim(results)[1]," good results.", sep = ''))

  working_account = "noone"
  if (at_least_one_result_from_cluster) {
